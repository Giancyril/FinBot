    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="relative w-full sm:w-48">
          <SlidersHorizontal size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
          <select value={category} onChange={(e) => setCategory(e.target.value)}
            className="w-full bg-gray-800/60 border border-white/5 text-gray-300 text-xs rounded-xl pl-9 pr-3 py-2.5 outline-none focus:border-white/10 transition-all appearance-none cursor-pointer">
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-gray-800/40 border-b border-white/5">
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-gray-500">Date</th>
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-gray-500">Merchant</th>
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-gray-500">Category</th>
              <th className="px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-gray-500 text-right">Amount</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {loading ? (
              <tr><td colSpan={4} className="py-12 text-center">
                <div className="w-5 h-5 border-2 border-indigo-400/40 border-t-indigo-400 rounded-full animate-spin mx-auto" />
              </td></tr>
            ) : transactions.length === 0 ? (
              <tr><td colSpan={4} className="py-12 text-center text-gray-600 text-sm">No transactions found</td></tr>
            ) : transactions.map((tx) => {
                ...
                <tr key={tx.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white text-xs font-medium truncate max-w-[160px] group-hover:text-indigo-300 transition-colors">
                  ...
                  <td className={`px-4 py-3 text-right text-xs font-bold ${isDebit ? 'text-red-400' : 'text-emerald-400'}`}>
